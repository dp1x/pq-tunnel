...
//! * `server` — v2 datagram server (ML-DSA-roster authenticated, fixed cover
//!   traffic); the legacy v1 QUIC runtime remains behind `--transport quic`
//!   until its scheduled removal.
//! * `client` — v2 datagram client (pinned server key); the legacy v1 QUIC
//!   runtime (flood/cover/tunnel modes) remains behind `--transport quic`.
...
pub struct ServerArgs {
    ...
    /// Attack-surface: stats reporting interval in seconds.
...
pub struct ClientArgs {
    ...
    /// TUN interface address in CIDR form (e.g. 10.0.0.1/24).
